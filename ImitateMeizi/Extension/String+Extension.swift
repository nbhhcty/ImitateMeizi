...
extension String {
    ...
    func getFirstChar() -> String {
        ...
        firstChar = "\(self[self.startIndex])"
        return firstChar
    }
    ...
    func encode() -> String {
        var str = self
        str = str.addingPercentEncoding(withAllowedCharacters: .urlFragmentAllowed)!
        return str
    }
    ...
    ///
    /// - Returns: 字典
    func toDic() -> [String: AnyHashable] {
        if let data = self.data(using: String.Encoding.utf8) {
            do{
                let json = try JSONSerialization.jsonObject(with: data,
                                                            options: .mutableContainers) as? [String: AnyHashable]
                if let dic = json{
                    return dic
                }
            }catch {
...
extension String {
    ...
    /// - Parameters:
    ///   - sub: 目标字符串
    ///   - backwards: 第一个，还是最后一个
    /// - Returns: （如果backwards参数设置为true，则返回最后出现的位置）
...
extension String {
    ...
    func substring(to: String) -> String {
        let index = self.indexOf(sub: to)
        return self.substring(from: nil, to: index)
    }
...
// MARK: - size
extension String {
    func contentSize(width:CGFloat, height:CGFloat,font:CGFloat) -> CGSize {
        if self.count <= 0 {
            return CGSize.zero
        }
        let size = CGSize(width: width, height: height)
        let options = NSStringDrawingOptions.usesFontLeading.union(.usesLineFragmentOrigin)
        let estimatedFrame = NSString(string: self).boundingRect(with: size, options: options, attributes: [NSAttributedStringKey.font:UIFont.systemFont(ofSize:font)], context: nil)
