...
typealias SavePhotoCallBack = (_ isSuccess: Bool)->()
fileprivate var saveResult: SavePhotoCallBack?

// MARK:保存图片到系统相册
extension UIImage {
    func saveToPhotoAlbum(callBack: @escaping SavePhotoCallBack) {
        saveResult = callBack
    ...
    /// - Parameters:
    ///   - image: 图片
    ///   - didFinishSavingWithError: 错误
    ///   - contextInfo: 上下文
    @objc func image(image: UIImage,
                     didFinishSavingWithError: NSError?,
                     contextInfo: AnyObject) {
        if let callback = saveResult {
            callback(didFinishSavingWithError == nil)
        }
    }
}

public extension UIImage {
    static func createQRCode(url: String?) -> UIImage {
        //创建滤镜
        ...
        if let outputImage = filter?.outputImage {
            //生成清晰度更好的二维码
            let qrCodeImage = UIImage.setupHighDefinitionUIImage(outputImage, size: 60)
            return qrCodeImage
        }
        return UIImage()
    }
...
public extension UIImage {
    ...
    public convenience init?(color: UIColor) {
        let rect = CGRect(x: 0,
                          y: 0,
                          width: 1 * UIImage.screenScale(),
                          height: 1 * UIImage.screenScale())
    ...
    private static func size(sizeString: String) -> CGSize {
        let array:[String] = sizeString.components(separatedBy: "x")
        guard array.count >= 2 else {
            return CGSize.zero
        }
        if let value1 = Float(array[0]), let value2 = Float(array[1]) {
            return CGSize(width: CGFloat(value1), height: CGFloat(value2))
        }
        else {
            return CGSize.zero
        }
    ...
    public func crop(in rect: CGRect) -> UIImage {
        guard let imageRef = cgImage?.cropping(to: CGRect(x: rect.origin.x * scale,
                                                          y: rect.origin.y * scale,
                                                          width: rect.size.width * scale,
                                                          height: rect.size.height * scale)) else {
            return self
        }
